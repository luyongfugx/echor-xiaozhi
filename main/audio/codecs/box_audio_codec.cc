                             ...
                             gpio_num_t pa_pin, uint8_t es8311_addr, uint8_t es7210_addr, bool input_reference)
{
    ...
    // Modified: Read 3 channels to support both DOA and AEC
    // - For DOA: use MIC1 (ch0) and MIC3 (ch2)
    // - For AEC: use MIC1 (ch0) and REF (ch3)
    // Original: input_channels_ = input_reference_ ? 2 : 1;
    input_channels_ = input_reference_ ? 3 : 2; // 3 channels: MIC1, MIC3, REF when AEC enabled
...
void BoxAudioCodec::EnableInput(bool enable)
{
    ...
    if (enable)
    {
        // Build channel mask: MIC1 (ch0) + MIC3 (ch2) + optionally REF (ch3)
        uint16_t mask = ESP_CODEC_DEV_MAKE_CHANNEL_MASK(0) | ESP_CODEC_DEV_MAKE_CHANNEL_MASK(2);
        if (input_reference_)
        {
            mask |= ESP_CODEC_DEV_MAKE_CHANNEL_MASK(3);
        }

        esp_codec_dev_sample_info_t fs = {
            .bits_per_sample = 16,
            .channel = 4,
            // Read MIC1 (ch0), MIC3 (ch2), and optionally REF (ch3) for AEC
            // - MIC1 + MIC3: for DOA (Direction of Arrival)
            // - MIC1 + REF: for AEC (Acoustic Echo Cancellation)
            // Note: MIC2 (ch1) appears to have no signal, possibly not connected
            .channel_mask = mask,
